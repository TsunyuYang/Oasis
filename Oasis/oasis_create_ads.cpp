...
void init(string path)
{
	...
	klogV = k * (long)(log2(num_nodes)+2) * 2; 
...
// g++ oasis_create_ads.cpp -lpthread -laio -O3
// g++ oasis_create_ads.cpp -lpthread -laio -o a-no-selec.out

// preprocessing:
// ./a.out 1 /home/yangty/graph_data/binary_graph/soc-LJ.bin /home/yangty/ADS/Oasis/data/soc-LJ.origin.8p/ 4847616 8 

// execution:
// sudo cp -r /home/yangty/ADS/Oasis/data/soc-LJ.origin.8p/ /data/nvme_raid/
// sudo nohup ./a.out 2 /data/nvme_raid/soc-LJ.origin.8p/ 16 16 1>nohup.oasis.soc-LJ.8p.k16.origin.create_ads.out 2>&1 &
// sudo rm -r /data/nvme_raid/soc-LJ.origin.8p/

// sudo ./a.out 2 /data/nvme_raid/soc-LJ.origin.8p/ 64 16
...
int main(int argc, char ** argv)
{
	...
	while(new_cnt > 0) {
		...
		for(int dst_pid = 0; dst_pid < num_part; ++dst_pid){
			...
			for(int src_pid = 0; src_pid < num_part; ++src_pid){
			...
			long bytes = my_fwrite(fd_ads[dst_pid], ads_buffer, io_size, 0);
			if (debugmode1){
				if (bytes != io_size){
